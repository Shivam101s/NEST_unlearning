...
def GA(model, data, loss, epoch, optimizer, scaler, scheduler, dist_model, args,
        ...
        for i, batch in enumerate(dataloader):
            ...
            with autocast():
                ...
                if split == 'forget' and not args.unlearn_method.endswith('_o'):
                    # cosine similarity loss (1 - cos) to push apart img/text on forget set
            ...
            # gradient ascent on forget split
            if split == 'forget':
    ...
    # --------- EVALUATION + LOGGING (updated return signature) ----------
    if is_master(args):
        (
            fgt1, fgt5,
            c1, c5,
