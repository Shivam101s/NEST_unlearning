...
"""
Neuron importance (S_i) with GLOBAL Stability Selection + per-layer FDR activation testing.

Fix for object concepts (e.g., school_bus):
- Hard (stable ∩ fdr) can be empty due to disjoint sets (common in text tower for objects).
- Use an adaptive combine rule:
    * If intersection has enough neurons -> keep it.
    * Else -> choose top-k by S from (stable ∪ fdr) (or stable first, then fill from fdr).

Outputs:
  results/neuron importance_global_sd/<concept>/<model>_<pretrained>_Si.json
  plus CSV summaries.
"""
    ...
    def _make_hook(self, lname):
        def hook(module, inp, out):
            ...
            with torch.no_grad():
                y = y.detach().float()
                if y.ndim == 4:      # [B,C,H,W]
                    v = y.reshape(y.shape[0], y.shape[1], -1).norm(p=2, dim=2).mean(dim=0)
                elif y.ndim == 3:    # [B,T,C]
                    v = y.reshape(-1, y.shape[-1]).norm(p=2, dim=0)
                elif y.ndim == 2:    # [B,C]
                    v = y.norm(p=2, dim=0)
                else:
                    return
...
# -----------------------------
# Prompt override (objects)
# -----------------------------
def build_forget_prompts(concept: str, use_templates: bool = True) -> List[str]:
    base = concept.replace("_", " ").strip()
    ...
    return [
        ...
        f"a picture of a {base}",
        f"a close-up photo of a {base}",
        f"a {base} in a street scene",
        f"a {base} on a road",
    ]

def override_forget_texts(tokenizer, concept: str, batch_size: int, use_templates: bool) -> torch.Tensor:
    prompts = build_forget_prompts(concept, use_templates=use_templates)
    chosen = [random.choice(prompts) for _ in range(batch_size)]
    return tokenizer(chosen)
...
def compute_Si_per_layer(
    ...
    for lname, Gf in forget_grads.items():
        if not is_weight_param(lname, Gf):
            continue
        if lname not in retain_grads or lname not in act_f_mean or lname not in act_r_mean:
            continue
        Gr = retain_grads[lname]
        if Gf.shape != Gr.shape:
            continue
        ...
        Af = act_f_mean[lname].to(Rf.device)
        Ar = act_r_mean[lname].to(Rf.device)
        if Af.numel() != Rf.numel() or Ar.numel() != Rf.numel():
            continue
...
def two_sample_t_pvalues(act_batches_f: torch.Tensor, act_batches_r: torch.Tensor) -> np.ndarray:
    ...
    df_den = (vx**2 / (Bf**2 * max(Bf - 1, 1))) + (vy**2 / (Br**2 * max(Br - 1, 1)))
...
def fdr_filter_indices(
    ...
    for lname, X in act_batches_f.items():
        if lname not in act_batches_r:
            out[lname] = np.zeros((0,), dtype=np.int64)
            continue
...
# -----------------------------
# Selection combine rule (FIX)
# -----------------------------
def choose_indices_adaptive(
    s: np.ndarray,
    stab: np.ndarray,
    fdr: np.ndarray,
    k_min_intersect: int,
    fallback_min: int,
    fallback_pct: float,
    mode: str = "adaptive_union_fill",
) -> Tuple[List[int], Dict[str, object]]:
    stab_set = set(stab.tolist())
    fdr_set  = set(fdr.tolist())
    inter = sorted(list(stab_set & fdr_set), key=lambda i: -s[i])

    info = {
        "combine_mode_used": None,
        "pool_size": None,
        "intersect_count": int(len(inter)),
    }

    if mode == "intersect_only":
        kept = inter
        info["combine_mode_used"] = "intersect_only"
        info["pool_size"] = int(len(inter))
        if len(kept) == 0:
            # fallback from top S
            cand = list(np.argsort(-s))
            k = max(fallback_min, int(round(fallback_pct / 100.0 * len(s))))
            kept = cand[:k]
            info["combine_mode_used"] = "fallback_topS"
            info["pool_size"] = int(len(cand))
        return kept, info

    # default: adaptive_union_fill
    if len(inter) >= k_min_intersect:
        kept = inter
        info["combine_mode_used"] = "intersect"
        info["pool_size"] = int(len(inter))
        return kept, info

    # union pool ranked by S
    pool = sorted(list(stab_set | fdr_set), key=lambda i: -s[i])
    info["pool_size"] = int(len(pool))
    ...
    # pick k based on "reasonable per-layer" rule
    k = max(fallback_min, int(round(fallback_pct / 100.0 * len(s))))
    # but if intersection was non-empty yet tiny, allow a bit more than min
    if len(inter) > 0:
        k = max(k, len(inter))

    kept = pool[:k]
    info["combine_mode_used"] = "union_topS"
    return kept, info
...
def run(args):
    ...
    pair_model = getattr(args, "model_name", args.model)
    ...
    def collect(split_key: str, max_batches: Optional[int]):
        loader = data[split_key].dataloader
        ...
        with torch.no_grad():
            for imgs, txts in loader:
                ...
                if split_key == "forget" and getattr(args, "override_forget_text", True):
                    txts = override_forget_texts(
                        tokenizer=tokenizer,
                        concept=args.celeb_name,
                        batch_size=imgs.size(0),
                        use_templates=getattr(args, "prompt_templates", True),
                    )
    ...
    for rd in range(args.stability_rounds):
        ...
        set_seed(getattr(args, "seed", 42) + 17 * rd)
        act_f_mean_r, _ = collect("forget", cap_f)
        act_r_mean_r, _ = collect("train",  cap_r)
    ...
    for lname, Svec in Si_base.items():
        s = Svec.numpy()
        stab = stable_idx.get(lname, np.array([], dtype=np.int64))
        fdr  = fdr_idx.get(lname,    np.array([], dtype=np.int64))

        kept, info = choose_indices_adaptive(
            s=s,
            stab=stab,
            fdr=fdr,
            k_min_intersect=int(getattr(args, "k_min_intersect", 8)),
            fallback_min=int(getattr(args, "fallback_min_per_layer", 8)),
            fallback_pct=float(getattr(args, "fallback_pct_per_layer", 0.2)),
            mode=str(getattr(args, "combine_mode", "adaptive_union_fill")),
        )

        final_idx[lname] = kept
        used_fallback = (info["combine_mode_used"] in ("fallback_topS",))

        meta[lname] = {
            "n": int(Svec.numel()),
            "stable_count": int(stab.size),
            "fdr_count": int(fdr.size),
            "intersect_count": int(info["intersect_count"]),
            "k_selected": int(len(kept)),
            "used_fallback": bool(used_fallback),
            "combine_mode_used": info["combine_mode_used"],
            "pool_size": int(info["pool_size"]) if info["pool_size"] is not None else 0,
    ...
    def make_ranked(scores: Dict[str, torch.Tensor], chosen: Dict[str, List[int]]):
        items = []
    ...
    ranked_vis = make_ranked(vis_scores, vis_final)
    ranked_txt = make_ranked(txt_scores, txt_final)
    ...
    out_json = {
        "config": {
            ...
            "fdr_q": args.fdr_q,
            "override_forget_text": getattr(args, "override_forget_text", True),
            "prompt_templates": getattr(args, "prompt_templates", True),
            "fallback_pct_per_layer": float(getattr(args, "fallback_pct_per_layer", 0.2)),
            "fallback_min_per_layer": int(getattr(args, "fallback_min_per_layer", 8)),
            "combine_mode": str(getattr(args, "combine_mode", "adaptive_union_fill")),
            "k_min_intersect": int(getattr(args, "k_min_intersect", 8)),
            "model_name": pair_model, "pretrained": pair_ckpt, "celeb_name": args.celeb_name,
        },
        "Si": {
            "vision":   {"ranked": ranked_vis},
            "language": {"ranked": ranked_txt},
        },
        "debug_meta": meta
    }
...
def main(cli_args=None):
    ...
    p.add_argument("--model_name", type=str, default="ViT-H-14")
    p.add_argument("--celeb_list", type=str, default="rifle")
    ...
    p.add_argument("--override_forget_text", action="store_true", default=True)
    p.add_argument("--no_override_forget_text", action="store_false", dest="override_forget_text")
    p.add_argument("--prompt_templates", action="store_true", default=True)
    p.add_argument("--no_prompt_templates", action="store_false", dest="prompt_templates")

    p.add_argument("--fallback_pct_per_layer", type=float, default=1)
    p.add_argument("--fallback_min_per_layer", type=int, default=32)

    # NEW: combine rule controls
    p.add_argument("--combine_mode", type=str, default="adaptive_union_fill",
                   choices=["adaptive_union_fill", "intersect_only"],
                   help="How to combine stable and FDR sets per layer.")
    p.add_argument("--k_min_intersect", type=int, default=8,
                   help="If |stable∩fdr| < this, use union-topS instead of intersection.")
    ...
    args.override_forget_text = known.override_forget_text
    args.prompt_templates = known.prompt_templates
    args.fallback_pct_per_layer = known.fallback_pct_per_layer
    args.fallback_min_per_layer = known.fallback_min_per_layer
    args.combine_mode = known.combine_mode
    args.k_min_intersect = known.k_min_intersect
